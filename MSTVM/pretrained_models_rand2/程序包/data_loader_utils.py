...
class MyDataset(Dataset):
    def __init__(self,dataset_choose, no_start, no_end):
        ...
        self.dataset_base_path = 'D:\PYTHON\迁移学习\王鑫-交接文件\迁移学习-交接文件\新2020-datasets'
        #self.scaler = scaler      
        ...
        elif self.dataset_choose[0:14] == 'Case_Rawsignal':    #'CasBearingFaultRaw': 
...
##########################################################################      
    def PlanetGearRaw4Ches_loader(self):        # 专门针对行星齿轮故障信号数据集的样本加载函数
        ...
        index = [x for x in range(0, len(sample))] 
...
##########################################################################  
    def CaseBearingFaultRaw_loader(self):        # 专门针对行星齿轮故障信号数据集的样本加载函数
        DATA = np.load(self.dataset_base_path + '/CaseBearingFaultRaw-原始数据-整理为字典/' + self.dataset_choose  + '.npz')
        sample = DATA['sample']
        label  = DATA['label']
        load   = DATA['load']
        # 数据筛选，筛选出部分特征的数据
