...
%Parameters of segmentation 分段参数
l=50;                  %the number of points inone period 每段的长度（按周期分段）
m=10;                  %the number of circulating shifts 循环平移需要的段数
n=1;                   %the number of points in one circulating shift(it can be set as 1 or 2.) 每次平移点数
%Parameters of adaptive K-SVD 自适应K-SVD参数(参照K-SVD函数)
param.K=300;
param.numIteration = 10;
param.errorFlag = 1;
param.errorGoal =0.3;                      %KSVD的参数，重点是errorGoal
param.preserveDCAtom =0;
param.displayProgress = 1;
param.InitializationMethod ='GivenMatrix';
k=290;                                      %the number of atoms 原信号中取出作为初始字典的原子个数

%% simulation signal仿真信号
