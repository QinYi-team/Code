...
% can sparsely represent each signal. 
% =========================================================================
% INPUT ARGUMENTS:
% Data                         an nXN matrix that contins N signals (Y), each of dimension n. 
...
if (~isfield(param,'displayProgress'))  %判定displayProgress是否已赋值，若没有则赋值为0
    param.displayProgress = 0;
end
totalerr(1) = 99999;                    %初始化总体误差
if (isfield(param,'errorFlag')==0)      %判定并赋值给errorGoal
    param.errorFlag = 0;
end

if (isfield(param,'TrueDictionary'))    %判定是否给出了真实字典
...
if (param.preserveDCAtom>0)             %判定preserveDCAtom
    FixedDictionaryElement(1:size(Data,1),1) = 1/sqrt(size(Data,1));    %若preserveDCAtom>0，那么将第一列赋值为常数并规范化
else
    FixedDictionaryElement = [];
end
% coefficient calculation method is OMP with fixed number of coefficients

if (size(Data,2) < param.K)         %如果字典元素个数大于数据的列数
...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        CoefMatrix = OMPerr([FixedDictionaryElement,Dictionary],Data, param.errorGoal);             %OMPerr调用
        param.L = 1;
    end
    %运用OMP算法生成初始字典下的系数矩阵
    replacedVectorCounter = 0;
	rPerm = randperm(size(Dictionary,2));               %将字典的序列打乱重新排列
    for j = rPerm                                       %依重新排列后的次序进行for循环
        [betterDictionaryElement,CoefMatrix,addedNewVector] = I_findBetterDictionaryElement(Data,...
            ...
            CoefMatrix ,param.L);
        Dictionary(:,j) = betterDictionaryElement;      %得到更好的字典原子
        if (param.preserveDCAtom)
...
if (length(relevantDataIndices)<1) %(length(relevantDataIndices)==0)
    ...
    betterDictionaryElement = betterDictionaryElement./sqrt(betterDictionaryElement'*betterDictionaryElement);%规范化
